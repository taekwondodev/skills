...
Consume the reviewed scope, verification evidence, issue reference, and approved actions from the task. For a standalone invocation, inspect the changes and establish that scope before staging. Load [delivery mode and branch preparation](../implement/references/delivery.md) to resolve any missing route or branch context. Reuse a current resolution rather than asking again.

Inspect `git status --porcelain`, `git diff HEAD`, `git diff --cached`, unstaged changes, and untracked files. Stop on conflicts or an in-progress Git operation. For an associated issue, read `docs/agents/issue-tracker.md` and fetch its body, comments, and state; ask only when the intended issue is ambiguous.
...
For an associated issue, append its identifier as a non-closing reference in the subject. Run `git commit -m "<message>"`. Verify the created revision, complete committed diff, and remaining worktree/index against the approved scope. If hooks changed the artifact, inspect and reverify affected behavior before delivery.
