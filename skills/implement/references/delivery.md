...
Load this reference before implementation or delivery when the task's route or branch is unresolved.
...
Record the route, qualified source and target refs, starting revision, and approved actions in the task record. A route is not authorization for delivery actions. Carry forward the user's requested scope, including any approvals or limits already given.

Completion: the route and refs are unambiguous, and requested actions are distinct from pending decisions.
